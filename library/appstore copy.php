...
function createHTMLAppStore($linkImg, $nomeApp, $classe, $descrizione, $autore,$linkautore,$categoriaApp, $prezzoApp, $linkAppStore, $rating){
if (is_feed()) {
...
<div class="review clearfix">
	...
	<div class="content">
		...
		<div class="field rating"><span class="icon" style="width:'.$rating.'%"></span></div>
	</div>
</div>';
}
		return $strHtml;
...
function createLinkAppStore($idApp,$atts){
	...
	if($obj->{'resultCount'} > 0){
		...
		if($atts["linkdownload"] == "") {$linkAppStore = 'http://clkuk.tradedoubler.com/click?p=24373&a=1972645&g=11695726&url='.$obj->results[0]->trackViewUrl;} else {$linkAppStore = 'http://clkuk.tradedoubler.com/click?p=24373&a=1972645&g=11695726&url='. $atts["linkdownload"]; }
		...
		if($atts["rating"] == "") {$rating = $obj->results[0]->averageUserRating * 20;} else {$rating = $atts["rating"]; }
		...
		$strHtml = createHTMLAppStore($linkImg, $nomeApp, $classe, $descrizione, $autore,$linkautore,$categoriaApp, $prezzoApp, $linkAppStore, $rating);
...
function market_code ($sav_atts, $sav_content = null) {
	...
	<div class="header">
		<h2 class="h2">App per Android</h2><h1 class="h1">'. $nome .'</h1><div class="download"><span class="icon"></span><a href="'. $link_market .'" target="_blank">Download</a></div>
	</div>
	<div class="content">
...
/*
Plugin Name: WINDOWS STORE APPS ex. http://apps.microsoft.com/windows/en-us/app/modern-reader/32341570-8307-4797-8d98-089e34d32eb0
*/

// Shortcode per link al market
function wptasc_get_file ($package_name) {
	$opts = array(
	'http'=>array(
	'method'=>"GET",
	'header'=>"User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1500.72 Safari/537.36\r\n" .
	...
	$wptasc_context = stream_context_create($opts);
	global $wptasc_file;
	$wptasc_file = false;
	
	// Open the file (webpage) using the HTTP header created above
	if ($package_name != ""){
		$wptasc_file = @file_get_contents('http://apps.microsoft.com/windows/it-it/app/' . $package_name, false, $wptasc_context);
		}
}

function market_code ($wptasc_atts, $wptasc_content = null) {
	extract( shortcode_atts( array(
		'type' => 'market',
		'recensione' => '',
	), $wptasc_atts ) );
	...
	global $wptasc_file;
	if ($wptasc_content != "") {
		wptasc_get_file($wptasc_content);

		if ($wptasc_file != false) {
			$regexp = "<span\sitemprop=\"offers\"\s[^>]*>.*itemprop=\"offerType\">\s<meta\scontent=\"(.*)\"\sitemprop=\"price\">\s<span\sitemprop=\"seller\"";
			if(preg_match_all("/$regexp/siU", $wptasc_file, $matches)) {
				$prezzo = $matches[1][0];
				}
			if ($prezzo == "ND") {
				$prezzo = "Free";
		...
		$link_market = "https://play.google.com/store/apps/details?id=" . $wptasc_content;
		$link_appbrain = "http://www.appbrain.com/app/" . $wptasc_content;
		if (strcasecmp($type, "market") == 0)
			$link_qr_code = $link_market;
		else if (strcasecmp($type, "appbrain") == 0)
			$link_qr_code = $link_appbrain;
...
function wptasc_qr_code ($qr_atts, $qr_content = null) {
	extract( shortcode_atts( array(
		'size' => '125',
		'type' => 'market',
		'class' => '',
...
function wptasc_star_code ($star_atts, $star_content = null) {
	extract( shortcode_atts( array(
		'class' => '',
	), $star_atts ) );
	...
	if ($wptasc_file == false){
		wptasc_get_file($star_content);
		}
	else {
		if (!stristr($wptasc_file, $star_content)){
			wptasc_get_file($star_content);
			}
		}
...
function wptasc_icon_code ($icon_atts, $icon_content = null) {
	extract( shortcode_atts( array(
		'size' => '125',
		'class' => '',
	), $icon_atts ) );
	...
	if ($wptasc_file == false){
		wptasc_get_file($icon_content);
		}
	else {
		if (!stristr($wptasc_file, $icon_content)){
			wptasc_get_file($icon_content);
			}
		}
	...
	if ($wptasc_file != false) {
		$regexp = "<div\sclass=\"doc-banner-icon\"><img\s[^>]*src=\"([^\"]*)\"";
			if(preg_match_all("/$regexp/siU", $wptasc_file, $matches)) {
				$wptasc_icon = $matches[1][0];
			}
		}
	if ($class != "") {
		return '<img src="' . $wptasc_icon . '" class="' . $class . '" alt="logo-app" />';
		}
	else {
		return '<img src="' . $wptasc_icon . '" width="' . $size . '" height="' . $size .'" alt="logo-app" />';
		}
}

add_shortcode( 'googleplay', 'market_code' );
add_shortcode( 'qr', 'wptasc_qr_code' );
add_shortcode( 'stars', 'wptasc_star_code' );
add_shortcode( 'appicon', 'wptasc_icon_code' );
