...
class instagramPhp{
     ...
     * you can use the $args array to pass the attributes that are used by the GET/users/user-id/media/recent method
     */
    public function getTag($args=array()){
        if($this->userid<=0){
            //If no user id, get user id
            $this->getUserIDFromUserName();
        }
        if($this->userid>0 && strlen($this->access_token)>0){
            $qs='';
            if(!empty($args)){ $qs = '&'.http_build_query($args); } //Adds query string if any args are specified
            $shots = $this->queryInstagram('https://api.instagram.com/v1/tags/tascspotted/media/recent?access_token='.$this->access_token.$qs); //Get shots
            if($shots->meta->code=='200'){
                return $shots;
            } else {
                $this->error('getUserMedia');
     ...
     */
    public function simpleDisplay($shots,$resolution = 'thumbnail',$showinfo = false){
        $simpleDisplay = '';
        if(!empty($shots->data)){
            $simpleDisplay.='<ul class="instagram_shots">';
                foreach($shots->data as $istg){
                    //The image
                    $istg_thumbnail = $istg->{'images'}->{$resolution}->{'url'}; //Thumbnail
                    $istg_standard = $istg->{'images'}->{'standard_resolution'}->{'url'}; //Standard
                    ...
                    //The markup
                    $simpleDisplay.='<li><a class="instalink" href="'.$istg_standard.'" target="_blank" title="'.$istg_caption.'"><img src="'.$istg_thumbnail.'" alt="'.$istg_caption.'" /></a>';
                    
                    if($showinfo == true) {
                    	$username = $istg->{'caption'}->{'from'}->{'username'};
                    	if(empty($username)) {$username = $istg->{'user'}->{'username'};}
                    	$simpleDisplay.='<span>by <a href="http://www.instagram.com/'.$username.'">'.$username.'</a></span></li>';
                    
                    } else {$simpleDisplay.='</li>';}
                }
            $simpleDisplay.='</ul>';
        } else {
